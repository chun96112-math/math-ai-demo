...
- **物件一致性原則**：一旦在對話中決定了使用的物件（例如蘋果），在接下來的互動中必須全程使用同一個物件，絕對不能中途隨意更換。
- **強制繪製分裝容器**：當題目討論到「裝成幾袋」或「分給幾個人」時，**務必使用矩形或虛線框**將每組物件明確框出來，清楚展現分組的概念。
- **精算數量與防呆**：必須精確計算總數（例如 每袋 5 個、裝 2 袋，總數是 10）。
  - **防止重疊 Bug**：當總數超過 12 個（例如 20 個）時，必須強制縮小點點的半徑（例如 r=8 改為 r=6），並增加排列的行數，確保所有點點都能整齊排列，**絕對不能發生重疊的情況**。
- 使用 <svg viewBox="0 0 450 220" width="100%" height="160" style="background:#fff; border-radius:8px;">。
- 圖片上方的標題文字與下方的說明文字，必須與實際畫出來的物件數量、分組框線完全一致。`;
